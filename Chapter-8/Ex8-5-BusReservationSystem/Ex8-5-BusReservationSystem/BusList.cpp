...
void BusList::writetoFile() {
	...
	file.open("data.txt", ofstream::trunc);

	for (auto it = this->Bus_List.begin(); it != this->Bus_List.end(); it++) {
		string str = to_string(it->getBusID()) + "," + it->getdriver() + "," + it->getarrivaltime() + "," + it->getdeparturetime() + "," + it->getdestination() + ",";
		file << str;

		int** seats = it->getseats();
		string** passengers = it->getpassengers();
		...
		for (int row = 0; row < it->getseat_rows(); row++) {
			for (int seat_in_row = 0; seat_in_row < it->getseats_per_row(); seat_in_row++) {
				if (passengers[row][seat_in_row] != " ") {
					string seat_pas = to_string(seats[row][seat_in_row]) + "-" + passengers[row][seat_in_row] + ";";
					file << seat_pas;
				}
			}
		}

		if (next(it) != Bus_List.end()) {
			cout << "Test";
			file << endl;
		}
